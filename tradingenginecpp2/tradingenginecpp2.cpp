...
int main()
{
    ...
    try {
        orderData.init_webSocket("stream.bybit.com", "443", "/v5/public/linear");
        if (orderData.is_socket_open()) {
            std::string subscription_message = R"({"op": "subscribe", "args": ["orderbook.200.ETHUSDT"]})";
            orderData.write_Socket(subscription_message);
        }
        while (true) {
            orderData.read_Socket();
            orderbook->HandleUpdate(orderData.get_socket_data());
            std::cout << orderbook->GetMarketPrice() << std:: endl;
            bybit.buffer_clear();
        }
